# mechanic_macos.py — Gunter AI Vanagon Diagnostic Command
# GUI: pure tkinter, LCARS-inspired theme matching luna_dash.py
# v2.0-mac — macOS port of Mechanic.py
#
# macOS-specific changes from Mechanic.py:
#   - BASE_PATH updated to /Users/minimac/PyCharmMiscProject/PythonAIAgent
#   - Audio: paplay replaced with afplay (macOS native)
#   - TTS: piper binary path updated for macOS (arm64 build)
#   - PDF viewer: evince replaced with macOS 'open' command
#   - CPU temp: /sys/class/thermal unavailable; reads macOS via osx-cpu-temp or smctemp
#   - Toplevel windows: lift()/focus_force()/attributes workaround for macOS z-order bug
#   - Picovoice wake word: uses macOS .ppn model file (different from Raspberry Pi)
#   - Window geometry: sized for Mac display (resizable), not locked to 1024x600
#   - Button colors: macOS Aqua theme ignores tk.Button bg/fg; fixed via ttk.Style
#     with a custom "LCARS.TButton" style that fully respects our palette
...
from tkinter import simpledialog, ttk  # noqa
import threading
import pvporcupine
import pyaudio
import requests
...
WAKE_MODE   = os.getenv("WAKE_MODE",   "text")   # default text on Mac — no Pi mic setup
LOCAL_MODEL = os.getenv("LOCAL_MODEL", "llama3.2:3b")
MIC_INDEX   = 0
MODEL_SIZE  = "base.en"

# ── Paths — macOS ─────────────────────────────────────────────────────────────
BASE_PATH           = "/Users/minimac/PyCharmMiscProject/PythonAIAgent"
...
def log_service(question, answer):
    ...
    entry = f"\n{'=' * 50}\nDATE: {timestamp}\nISSUE: {question}\nGUNTER: {answer}\n"
    log_path = os.path.join(BASE_PATH, "service_history.txt")
...
def play_ready_sound():
    """Play ding.wav using afplay (macOS native audio player)."""
    ding_path = f"{BASE_PATH}/ding.wav"
...
def speak(text):
    ...
    # macOS piper binary — arm64 build expected at BASE_PATH/piper/piper
    piper_cmd    = f"{BASE_PATH}/gunter_venv/bin/piper"
...
def _open_manual_to_page(page_num, source="bentley"):
    """
    Open PDF to a specific page on macOS.
    macOS 'open' command can't jump to a page directly, so we open the PDF
    and print a console note about the page. For page-specific navigation,
    users can use Preview's Go To Page (Cmd+G) feature.
    Falls back to opening the full PDF if page-jump isn't supported.
    """
    manual_path = DIGIFANT_PATH if "digifant" in source.lower() else BENTLEY_PATH
    try:
        # Try to open with Preview at a specific page via AppleScript
        script = (
            f'tell application "Preview" to open POSIX file "{manual_path}"\n'
            f'delay 1\n'
            f'tell application "Preview" to activate'
        )
        subprocess.Popen(["osascript", "-e", script])
        print(f"[!] Gunter opening {manual_path} — navigate to page {page_num} (Cmd+G in Preview)")
    except Exception as e:
        print(f"Error opening PDF: {e}")
        try:
            subprocess.Popen(["open", manual_path])
        except Exception as e2:
            print(f"Fallback open also failed: {e2}")
...
def _identify_systems(question):
    ...
    system_keywords = {
        ...
        "suspension":   ["bearing", "wheel bearing", "tire", "tyre", "pressure",
                         ...
                         "front wheel", "rear wheel", "cv joint", "cv boot",
                         "axle", "clicking", "clunk"],
...
# macOS Aqua theme silently ignores bg/fg on tk.Button.
# Solution: ttk.Button with a named style. ttk styles ARE respected on macOS.
# We define one style per color variant used across the UI.

def _init_button_styles(root):
    """
    Call once after the root Tk window exists.
    Defines all LCARS button style variants used in the UI.
    """
    s = ttk.Style(root)
    # Use 'clam' as the base — it fully supports background/foreground on all platforms
    s.theme_use("clam")

    # ── Style map helper ──────────────────────────────────────────────────────
    def _make_style(name, bg, fg, active_bg, active_fg=None):
    ...
    _make_style("Blue.LCARS.TButton",    FRAME_BLUE, BG,       FRAME_DIM)
    _make_style("Dim.LCARS.TButton",     FRAME_DIM,  GOLD,     FRAME_BLUE)
    _make_style("DimAmber.LCARS.TButton",FRAME_DIM,  AMBER,    FRAME_BLUE)
    _make_style("DimCyan.LCARS.TButton", FRAME_DIM,  CYAN,     FRAME_BLUE)
    _make_style("Gold.LCARS.TButton",    GOLD_DIM,   BG,       GOLD)
    _make_style("RedAlert.LCARS.TButton",RED_ALERT,  RED_BRIGHT, RED_BRIGHT, GOLD)
    _make_style("DimGold.LCARS.TButton", FRAME_DIM,  CYAN,     FRAME_BLUE)

    # Smaller font variants for modal buttons
    s.configure("Small.Blue.LCARS.TButton",
                background=FRAME_BLUE, foreground=BG,
                font=("Courier", 9, "bold"), relief="flat",
                borderwidth=0, padding=(6, 2), focuscolor=FRAME_BLUE)
    s.map("Small.Blue.LCARS.TButton",
          background=[("active", FRAME_DIM)],
          foreground=[("active", GOLD)])

    s.configure("Small.Dim.LCARS.TButton",
                background=FRAME_DIM, foreground=GOLD,
                font=("Courier", 9, "bold"), relief="flat",
                borderwidth=0, padding=(6, 2), focuscolor=FRAME_DIM)
    s.map("Small.Dim.LCARS.TButton",
          background=[("active", FRAME_BLUE)],
          foreground=[("active", GOLD)])

    s.configure("Small.DimCyan.LCARS.TButton",
                background=FRAME_DIM, foreground=CYAN,
                font=("Courier", 9, "bold"), relief="flat",
                borderwidth=0, padding=(8, 2), focuscolor=FRAME_DIM)
    s.map("Small.DimCyan.LCARS.TButton",
          background=[("active", FRAME_BLUE)],
          foreground=[("active", GOLD)])

    s.configure("Small.RedAlert.LCARS.TButton",
                background=RED_ALERT, foreground=RED_BRIGHT,
                font=("Courier", 10, "bold"), relief="flat",
                borderwidth=0, padding=(8, 3), focuscolor=RED_ALERT)
    s.map("Small.RedAlert.LCARS.TButton",
          background=[("active", RED_BRIGHT)],
          foreground=[("active", GOLD)])

    s.configure("Small.GoldDim.LCARS.TButton",
                background=GOLD_DIM, foreground=BG,
                font=("Courier", 9, "bold"), relief="flat",
                borderwidth=0, padding=(8, 2), focuscolor=GOLD_DIM)
    s.map("Small.GoldDim.LCARS.TButton",
          background=[("active", GOLD)],
          foreground=[("active", BG)])

    s.configure("Tab.Active.LCARS.TButton",
                background=FRAME_BLUE, foreground=BG,
                font=("Courier", 10, "bold"), relief="flat",
                borderwidth=0, padding=(20, 6), focuscolor=FRAME_BLUE)
    s.map("Tab.Active.LCARS.TButton",
          background=[("active", FRAME_DIM)],
          foreground=[("active", GOLD)])

    s.configure("Tab.Inactive.LCARS.TButton",
                background=FRAME_DIM, foreground=TEXT_SUB,
                font=("Courier", 10, "bold"), relief="flat",
                borderwidth=0, padding=(20, 6), focuscolor=FRAME_DIM)
    s.map("Tab.Inactive.LCARS.TButton",
          background=[("active", FRAME_BLUE)],
          foreground=[("active", GOLD)])
...
def _make_button(parent, text, command,
    ...
    """
    Create a color-correct button on macOS using ttk.Button + LCARS style.
    Selects the right named style based on the bg/fg combo requested.
    """
    # Map bg/fg combinations to named styles
    style_map = {
        (FRAME_BLUE, BG):    "Blue.LCARS.TButton",
        (FRAME_DIM,  GOLD):  "Dim.LCARS.TButton",
        (FRAME_DIM,  AMBER): "DimAmber.LCARS.TButton",
        (FRAME_DIM,  CYAN):  "DimCyan.LCARS.TButton",
        (GOLD_DIM,   BG):    "Gold.LCARS.TButton",
        (RED_ALERT,  RED_BRIGHT): "RedAlert.LCARS.TButton",
    }
    style = style_map.get((bg, fg), "Blue.LCARS.TButton")
...
# ── macOS Toplevel helper — fixes popup-behind-main-window bug ────────────────
def _raise_window(win):
    """
    Force a Toplevel window to the front on macOS.
    The sequence lift() → attributes topmost → update → focus_force is the
    reliable pattern for macOS tkinter z-order issues.
    """
    win.lift()
...
class GunterGUI(tk.Tk):
    def __init__(self):
        ...
        self.geometry("1100x700")          # Mac-friendly default size
        self.configure(bg=BG)
        self.resizable(True, True)

        # Bring main window to front on macOS
        self.lift()
        self.attributes("-topmost", True)
        self.after(200, lambda: self.attributes("-topmost", False))

        # Initialize ttk button styles — must happen after Tk root exists
        _init_button_styles(self)
    ...
    def _build_layout(self):
        ...
        ttk.Button(title_bar, text="EXIT SHOP",
                   command=self.quit_gunter,
                   style="RedAlert.LCARS.TButton").pack(side="right", padx=(0, 8))
    ...
    def _build_sidebar(self, parent):
        ...
        self.temp_label = tk.Label(bottom, text="CPU: --°C",
                                   fg=TEXT_SUB, bg=BG,
                                   font=("Courier", 10))
    ...
    def _build_chat(self, parent):
        ...
        # Right-click context menu — macOS also recognizes Button-2
        ...
        self.textbox.bind("<Button-2>", _show_context_menu)   # macOS two-finger/right-click
        self.textbox.bind("<Button-3>", _show_context_menu)   # also bind Button-3
    ...
    def open_bentley_manual(self):
        ...
        subprocess.Popen(["open", BENTLEY_PATH])   # macOS: 'open' replaces evince
    ...
    def show_service_history(self):
        ...
        # macOS: force window to front after a brief delay for grab_set to settle
        self.after(50, lambda: _raise_window(modal))
        ...
        ttk.Button(hdr, text="✕ CLOSE", command=modal.destroy,
                   style="Small.RedAlert.LCARS.TButton").pack(side="right", padx=12)
        ...
        ttk.Button(date_row, text="TODAY",
                   ...
                   style="Small.Blue.LCARS.TButton").pack(side="left")
        ...
        ttk.Button(status_frame, text="SAVE STATUS",
                   command=_save_status,
                   style="Small.Blue.LCARS.TButton").pack(anchor="e", padx=8, pady=(2, 6))
        ...
        for item in SERVICE_ITEMS:
            ...
            ttk.Button(row, text="EDIT",
                       ...
                       style="Small.Dim.LCARS.TButton").pack(side="right", padx=4)
    ...
    def _edit_service_record(self, key, label, parent_modal):
        ...
        # macOS: raise sub-dialog on top of parent modal
        self.after(50, lambda: _raise_window(dialog))
        ...
        ttk.Button(btn_row, text="SAVE",
                   command=_save,
                   style="Blue.LCARS.TButton").pack(side="left", padx=8)
        ttk.Button(btn_row, text="CANCEL",
                   command=_cancel,
                   style="Dim.LCARS.TButton").pack(side="left", padx=8)
    ...
    def show_settings(self):
        ...
        def _show_tab(name):
            ...
            tab_voice_btn.configure(style="Tab.Inactive.LCARS.TButton")
            tab_provider_btn.configure(style="Tab.Inactive.LCARS.TButton")
            tab_pico_btn.configure(style="Tab.Inactive.LCARS.TButton")
            if name == "voice":
                voice_frame.pack(fill="both", expand=True)
                tab_voice_btn.configure(style="Tab.Active.LCARS.TButton")
            elif name == "providers":
                provider_frame.pack(fill="both", expand=True)
                tab_provider_btn.configure(style="Tab.Active.LCARS.TButton")
            elif name == "picovoice":
                picovoice_frame.pack(fill="both", expand=True)
                tab_pico_btn.configure(style="Tab.Active.LCARS.TButton")

        tab_voice_btn = ttk.Button(
            tab_bar, text="VOICE",
            command=lambda: _show_tab("voice"),
            style="Tab.Active.LCARS.TButton")
        tab_voice_btn.pack(side="left", padx=(12, 2), pady=6)

        tab_provider_btn = ttk.Button(
            tab_bar, text="API PROVIDERS",
            command=lambda: _show_tab("providers"),
            style="Tab.Inactive.LCARS.TButton")
        tab_provider_btn.pack(side="left", padx=2, pady=6)

        tab_pico_btn = ttk.Button(
        ...
        v_canvas    = tk.Canvas(voice_frame, bg=BG, highlightthickness=0, height=300)
        ...
        for vname in VOICE_NAMES:
            ...
            accent = tk.Frame(row, bg=GOLD if is_current else FRAME_BLUE, width=5)
        ...
        ttk.Button(v_btn_row, text="▶ TEST VOICE",
                   command=_test_voice,
                   style="Small.DimCyan.LCARS.TButton").pack(side="left", padx=(0, 6))

        ttk.Button(v_btn_row, text="SAVE VOICE",
                   command=_save_voice,
                   style="Small.Blue.LCARS.TButton").pack(side="left")
        ...
        def _rebuild_provider_tab():
            ...
            for p in pdata["providers"]:
                ...
                key_display = ("●" * 8 + p["api_key"][-4:]) if len(p.get("api_key","")) > 4 else (
                    "No key set" if not p.get("api_key") else p["api_key"])
                tk.Label(info, text=f"Key: {key_display}  Model: {p.get('model','—')}",
                ...
                def _make_edit_cmd(provider_data):
                    return lambda: _open_provider_edit(provider_data, pdata)

                ttk.Button(btn_col, text="EDIT",
                           command=_make_edit_cmd(p),
                           style="Small.DimCyan.LCARS.TButton").pack(pady=2)

                if not is_active:
                    def _make_activate_cmd(pname):
                        def _activate():
                            pdata["active"] = pname
                        ...
                        return _activate
                    ttk.Button(btn_col, text="SET ACTIVE",
                               command=_make_activate_cmd(p["name"]),
                               style="Small.GoldDim.LCARS.TButton").pack(pady=2)

            # Bottom: active provider summary
            tk.Frame(provider_frame, bg=FRAME_BLUE, height=1).pack(
                fill="x", padx=14, pady=(10, 4))
            _ap = get_active_provider()
            tk.Label(provider_frame,
                     text=f"Active: {PROVIDER_LABELS.get(_ap['name'], _ap['name'])}  "
                          f"Model: {_ap.get('model','—')}",
                     fg=CYAN, bg=BG,
                     font=("Courier", 9)).pack(anchor="w", padx=14)

        def _open_provider_edit(p, pdata):
            ...
            ey = modal.winfo_y() + (modal.winfo_height() // 2) - 210
            edit.geometry(f"480x420+{ex}+{ey}")

            # macOS: raise edit dialog on top
            modal.after(50, lambda: _raise_window(edit))

            tk.Frame(edit, bg=RED_ALERT, height=3).pack(fill="x")
            tk.Label(edit,
                     text=PROVIDER_LABELS.get(provider_name, provider_name),
                     fg=GOLD, bg=BG,
                     font=("Courier", 13, "bold")).pack(pady=(10, 6))
            tk.Frame(edit, bg=FRAME_BLUE, height=1).pack(fill="x", pady=(0, 10))
            ...
            tk.Label(model_row, text="MODEL:", fg=GOLD_DIM, bg=BG,
                     font=("Courier", 9), anchor="w").pack(anchor="w")
            model_var = tk.StringVar(value=p.get("model", ""))

            fetch_status = tk.Label(body, text="", fg=AMBER, bg=BG,
                                    font=("Courier", 8))
            fetch_status.pack(anchor="w", pady=(0, 4))

            # Searchable model listbox
            ...
            tk.Label(search_frame, text="filter", fg=GOLD_DIM, bg=BG,
            ...
            list_scroll = tk.Scrollbar(list_frame, orient="vertical",
                                       bg=FRAME_DIM, troughcolor=BG)
            model_listbox = tk.Listbox(list_frame, height=6,
                                       bg=FRAME_DIM, fg=TEXT_MAIN,
                                       selectbackground=FRAME_BLUE,
                                       selectforeground=GOLD,
                                       font=("Courier", 9),
                                       relief="flat",
                                       yscrollcommand=list_scroll.set)
            list_scroll.config(command=model_listbox.yview)
            list_scroll.pack(side="right", fill="y")
            model_listbox.pack(side="left", fill="x", expand=True)

            select_row = tk.Frame(body, bg=BG)
            select_row.pack(fill="x", pady=(0, 4))
            tk.Label(select_row, text="Selected:", fg=TEXT_SUB, bg=BG,
                     font=("Courier", 9)).pack(side="left")
            selected_lbl = tk.Label(select_row, textvariable=model_var,
                                    fg=GOLD, bg=BG,
                                    font=("Courier", 9))
            selected_lbl.pack(side="left", padx=6)
            ...
            ttk.Button(select_row, text="USE SELECTED",
                       command=_commit_selection,
                       style="Small.DimCyan.LCARS.TButton").pack(side="left")
            ...
            ttk.Button(model_row, text="FETCH MODELS",
        ...
        ttk.Button(pv_body, text="SAVE KEY",
                   command=_save_pv_key,
                   style="Blue.LCARS.TButton").pack(anchor="w")
        ...
        tk.Label(picovoice_frame,
                 ...
                 font=("Courier", 8)).pack(anchor="w", padx=14)
        tk.Label(picovoice_frame,
                 text="Note: macOS requires the macOS .ppn wake word file,",
                 fg=AMBER, bg=BG,
                 font=("Courier", 8)).pack(anchor="w", padx=14)
        tk.Label(picovoice_frame,
                 text="not the Raspberry Pi version.",
                 fg=AMBER, bg=BG,
                 font=("Courier", 8)).pack(anchor="w", padx=14)
    ...
    def show_vehicle_config(self):
        ...
        def _save():
            self._van_config = {
                ...
                "voice_name": self._van_config.get("voice_name", "L2Arctic — Speaker 0"),
            }
        ...
        ttk.Button(modal, text="SAVE & CLOSE",
                   command=_save,
                   style="Blue.LCARS.TButton").pack(pady=10)

    # ── CPU temperature — macOS ───────────────────────────────────────────────
    def update_temp(self):
        """
        Read CPU temperature on macOS.
        Tries 'osx-cpu-temp' CLI first (brew install osx-cpu-temp),
        then 'smctemp' (brew install smctemp), then shows N/A gracefully.
        """
        temp = None
        for cmd in [["osx-cpu-temp", "-C"], ["smctemp", "-c"]]:
            try:
                result = subprocess.run(cmd, capture_output=True, text=True, timeout=2)
                raw = result.stdout.strip()
                # Output is typically "42.5°C" or "42.5 °C" — extract number
        ...
        if temp is not None:
            self.temp_label.config(
                ...
                fg=RED_BRIGHT if temp > 90 else TEXT_SUB
            )
        else:
            self.temp_label.config(text="CPU: N/A", fg=TEXT_SUB)

        self.after(10000, self.update_temp)  # poll every 10s (less aggressive on Mac)
    ...
    def _record_question(self, recorder):
        ...
        wav_path = os.path.join(BASE_PATH, "gunter_temp.wav")
        ...
        if WAKE_MODE == "picovoice":
            ...
            try:
                porcupine = pvporcupine.create(
                    access_key=_pv_key,
                    keyword_paths=[
                        # macOS: use the macOS .ppn file, not Raspberry Pi
                        f"{WAKE_WORD_PATH}/Hey-Gunter_en_mac_v4_0_0.ppn"
                    ]
                )
                recorder = PvRecorder(
                ...
                self.update_text(
                    f"Picovoice Error: {e}\n"
                    f"Note: macOS requires the macOS .ppn file (not Raspberry Pi).\n"
